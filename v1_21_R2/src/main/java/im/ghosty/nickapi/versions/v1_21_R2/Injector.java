package im.ghosty.nickapi.versions.v1_21_R2;
								...
								if (receivedPlayer.isOnline()) {
									...
									if (user != null) {
										if (receivedProfile.getId() != player.getUniqueId()) {
											...
											final ClientboundPlayerInfoUpdatePacket.Entry newInfoData = new ClientboundPlayerInfoUpdatePacket.Entry(receivedProfile.getId(), profile, infoData.listed(), infoData.latency(), infoData.gameMode(), infoData.displayName(), infoData.listOrder(), infoData.chatSession());
