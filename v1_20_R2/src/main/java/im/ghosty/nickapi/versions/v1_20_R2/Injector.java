package im.ghosty.nickapi.versions.v1_20_R2;
...
public class Injector implements IInject, Listener {
		...
		final ChannelDuplexHandler duplexHandler = new ChannelDuplexHandler() {
			public void write(final ChannelHandlerContext ctx, final Object packet, final ChannelPromise promise) throws Exception {
				if (packet instanceof ClientboundPlayerInfoUpdatePacket) {
					final ClientboundPlayerInfoUpdatePacket infoPacket = (ClientboundPlayerInfoUpdatePacket) packet;
	...
	@Override
	public void applyGameProfile(final Player player) {
		...
		String value = "";
		String signature = "";
		for (final Property property : craftPlayer.getProfile().getProperties().get("textures")) {
	...
	private ChannelPipeline pipeline(final Player player) {
		final ServerGamePacketListenerImpl serverGamePacketListener = ((CraftPlayer) player).getHandle().connection;
		try {
			final Field field = serverGamePacketListener.getClass().getSuperclass().getDeclaredField("c");
			field.setAccessible(true);
			final Connection connectionField = (Connection) field.get(serverGamePacketListener);
